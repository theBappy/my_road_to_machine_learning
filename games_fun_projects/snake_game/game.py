...
class Game:
    ...
    def is_collision(self):
        if(
            self.snake.head.x > self.display.width - self.snake.block_size or self.snake.head.x < 0 or self.snake.head.y > self.display.height - self.snake.block_size
    ...
    def place_food(self):
        x = (
            random.randint(
                0,
                (self.display.width - GameSettings.BLOCK_SIZE) // GameSettings.BLOCK_SIZE,
            )
        ...
        y = (
            random.randint(
                0,
                (self.display.height - GameSettings.BLOCK_SIZE) // GameSettings.BLOCK_SIZE,
            )
            * GameSettings.BLOCK_SIZE
        )
        self.food = Point(x, y)
